...
public class UserMediatorTest {
    ...
    public void testGetAll() {
        ...
        filter.setLimit(5);
        filter.setOffset(10);
        filter.setOrderBy("username");
        final User user = User.builder().username("username").build();
        final long totalCount = 16L;
        when(dao.find(filter.getFilter(), filter.getLimit(), filter.getOffset(), filter.getOrderBy()))
                .thenReturn(asList(user));
        when(dao.count(filter.getFilter())).thenReturn(totalCount);
